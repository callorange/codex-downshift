...
- **오버엔지니어링 경계**: 현재 요구사항과 제약을 충족하는 가장 단순하고 안전한 해결책을 우선하십시오. 현재 요구사항에 없는 미래 확장 가능성만을 이유로 불필요한 추상화·계층·일반화를 미리 추가하지 마십시오. 범용 기능은 직접 구현을 기본값으로 삼지 말고 기존 프로젝트의 의존성과 관례를 우선하며, 적합한 라이브러리가 구현량·검증 범위·유지보수 책임을 줄이는 경우 활용하십시오.
...
- **불명확한 오류 및 논리적 에러**:
  원인이 불명확하거나, 여러 원인이 가능하거나, 수정 후에도 실패가 지속되거나, 런타임/테스트 상의 예상과 다른 논리적 오류인 경우에는 [증거 기반 디버깅](#-4-증거-기반-디버깅-structured-troubleshooting)에 따라 원인 가설 수립과 증거 검증을 거쳐 최소 수정합니다.
...
## 🧪 3. 의미 있는 테스트 및 외부 경계 격리

- **관찰 가능한 동작 검증**:
  테스트는 검증문 없이 실행 여부만 확인하거나 무조건 성공하도록 작성하지 않습니다.
  대상 동작의 결과, 상태 변화, 오류 또는 side effect를 관찰 가능한 assertion으로 검증합니다.
- **기본 테스트의 외부 환경 독립성**:
  기본 자동 테스트는 network, 프로젝트가 통제하지 않는 외부 서비스의 상태 및 실제 credential 없이 반복 실행할 수 있어야 합니다.
  외부 서비스와의 I/O는 해당 경계의 계약을 재현하는 fake 또는 mock으로 대체합니다.
- **내부 계약의 실제 검증**:
  테스트 대상인 business logic, 상태 전이, 권한 정책, 영속화 결과 및 transaction 동작을 mock으로 우회하지 않습니다.
  DB 동작이 검증 대상에 포함되면 프로젝트가 지원하는 테스트 DB와 실제 ORM·transaction 경로를 사용합니다.
- **Test double의 적용 경계**:
  fake 또는 mock은 현재 테스트의 검증 대상이 아닌 외부 경계를 격리하는 데 사용합니다.
  테스트 대상 자체나 검증하려는 내부 계약을 대체하여 실제 결함을 숨기지 않습니다.
- **실제 외부 연결의 opt-in 분리**:
...
- **진입 조건 (Activation Conditions)**:
  - 도구의 진단 출력만으로 원인과 수정 방법이 결정적인 기계적 오류는 별도의 가설 수립 루프 없이 직접 수정하거나 프로젝트 도구(formatter/safe auto-fix)로 신속히 해결합니다.
  - 오류 원인이 불명확하거나, 여러 원인이 가능하거나, 수정 후에도 실패가 지속되거나, 런타임/논리적 동작이 예상과 다른 경우에만 지레짐작 수정을 방지하기 위해 다음 루프를 준수하십시오:
...
- **문서 부채 방지 및 동기화 시점**:
  주요 기능 추가, 아키텍처 개정, 환경 설정 변경 시 매 파일 편집마다 문서를 수정하는 마이크로 루프를 돌지 않고, 논리적 변경 단위의 코드 구현이 안정된 시점에 영향받는 기존 문서(README.md, AGENTS.md 등)를 확인하여 동일한 작업 변경(커밋/완료 전) 안에서 수술적 편집(Surgical Edit)으로 동기화합니다.
  문서 변경을 별도 후속 작업으로 미루지 않으며, 코드와 관련 문서는 최종적으로 일치해야 합니다.
...
- public API 문서와 코드 내 설명은 동일 작업 단위에서 최종 일치시키며, 관련 규칙 원본이 바뀌면 링크·목차·CHANGELOG와 빌드로 생성되는 배포 아티팩트도 함께 동기화합니다.
