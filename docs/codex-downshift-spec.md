...
#### 3) 토큰·단가 결합 4대 운용 규칙
- **Luna-First 원칙 (99% 절감)**: Luna `low`(지수 30)는 Sol Medium(지수 3,075) 대비 **1/102.5** 수준으로 비용이 증발한다. 10줄 이상의 기계적 구현/테스트는 무조건 Luna `low`를 디폴트로 호출한다.
- **Luna Medium(106) vs Terra Medium(738)**: 순수 토큰량은 Terra가 적지만 단가가 10배 차이나므로, 실제 크레딧은 **Luna Medium이 Terra Medium보다 7배 저렴**하다. 경량 로컬 탐색도 Luna Medium을 우선 고려한다.
- **Terra의 역할 (75% 절감형 고지능 워커)**: Terra는 Sol 대비 약 76%를 아끼는 모델로서, Luna의 지능으로 감당하기 힘든 복잡한 로컬 알고리즘/클래스 구조 설계에 한해 선택적으로 투입한다.
- **High / Max 자동 선택 절대 금지**: 토큰 급증과 비용 역전을 방지하기 위해 사용자 사전 승인 없이 `high` 이상 자동 선택을 금지한다.
