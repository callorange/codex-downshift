...
본 문서는 `codex-downshift` 스킬의 8대 핵심 동작 및 검증 시나리오를 정의한 참조 가이드입니다.
...
| :--- | :--- | :--- | :--- | :--- |
| **1** | 명확한 구현 / TDD 작업 | **Sol** | ✅ **위임 (Downshift)** | `gpt-5.6-luna` child 생성 (`fork_turns="none"`) |
| **2** | 명확한 문서 / 린트 수정 | **Terra** | ✅ **위임 (Downshift)** | `gpt-5.6-luna` child 생성 (`reasoning_effort="low"`) |
| **3** | 주력 모델로 단독 실행 중 | **Luna** | ❌ **비활성화 (No-op)** | child 생성 없이 Luna가 직접 처리 |
| **4** | 계획 / 브레인스토밍 요청 | **Sol/Terra** | ❌ **비활성화 (No-op)** | child 생성 없이 부모가 직접 설계·답변 |
| **5** | 원인 불명의 복잡한 디버깅 | **Sol/Terra** | ❌ **비활성화 (No-op)** | child 생성 없이 부모가 원인 규명 및 해결 |
| **6** | 새 설계 판단 직면 | **Luna Worker**| 🛑 **에스컬레이션** | `NEEDS_PARENT_DECISION` 반환 후 부모가 판단 |
| **7** | Luna spawn 실패 / 미지원 | **Sol/Terra** | 🛡️ **Fallback Invariant** | 상위 모델 child 생성 없이 부모가 직접 수행 |
| **8** | Luna 작업 완료 후 | **Sol/Terra** | 🔄 **최소 Acceptance** | 전체 재작업/동일테스트 반복 없이 결과만 확인 |
...
## 🧪 Scenario 3: Luna Parent 사용 시 (자동 위임 비활성화)
...
## 🧪 Scenario 4: Planning / Brainstorming 요청 (위임 제외)
...
## 🧪 Scenario 5: 원인 불명의 복잡한 디버깅 (위임 제외)
...
## 🧪 Scenario 6: Luna 작업 중 새 설계 판단 필요 시 (`NEEDS_PARENT_DECISION`)
...
## 🧪 Scenario 7: Luna Spawn 실패 시 Fallback (불변 규칙)
...
## 🧪 Scenario 8: Parent의 효율적인 결과 확인 (중복 작업 방지)
