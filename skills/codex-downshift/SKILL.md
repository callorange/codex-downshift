...
7. **Reasoning Effort & 실질 요율 정책 (No automatic above medium & Luna-First)**:
   - **Luna**: 기본값 `low` (Light: 1.00×, Sol Medium 18.04× 대비 **94.5% 절감**). 경량 탐색 시 `medium` (2.61×, Terra Medium 대비 2배 이상 저렴)까지 허용.
   - **Terra**: `low` (4.46×) 또는 `medium` (5.35×) 허용 (기본값: `medium`, Sol Medium 대비 **70.3% 절감**).
   - **골든 스위칭 규칙**: `Luna High (6.00×)`는 `Terra Medium (5.35×)`보다 비싸므로 사용 금지 ➔ 복잡한 작업은 더 똑똑하고 저렴한 Terra Medium으로 즉시 스위칭.
...
1. **Luna Light (1.00×) 디폴트 위임 (실질 94.5% 절감)**:
   - Sol Medium(18.04×) 대비 실질 비용은 **1/18 (94.5% 절감)**입니다. 기계적 구현/테스트 작업은 Luna `low`를 디폴트로 호출합니다.
2. **Luna Medium(2.61×)이 Terra Medium(5.35×)보다 2배 이상 저렴**:
   - 경량 로컬 탐색이나 중간 규모 수정은 Terra로 올리지 않고 Luna Medium(2.61×)을 우선 활용하여 비용을 51% 추가 절감합니다.
3. **골든 스위칭 규칙: `Luna High (6.00×)` > `Terra Medium (5.35×)`**:
   - 하위 모델인 Luna에게 High 추론(6.00×)을 부여하는 것은 **비용도 더 비싸고 지능도 떨어지는 최악의 비효율**입니다.
   - Luna Medium(2.61×)으로 감당하기 힘든 작업은 Luna High로 올리지 않고, **더 똑똑하면서 11% 더 저렴한 `Terra Medium (5.35×)`으로 즉시 스위칭**합니다!
4. **Terra Medium (5.35×)의 정체성 (실질 70.3% 절감형 고지능 워커)**:
   - 복잡한 로컬 알고리즘, 클래스 구조 설계에 한해 선택적으로 투입하여 Sol Medium 대비 70.3% 절감을 달성합니다.
5. **세션 누적 캐시 비용의 압도적 방어**:
   - Sol 메인 세션이 길어지면(40k 토큰) 매 턴 캐시 읽기에만 **400,000 크레딧**이 소모됩니다. 반면 Luna의 독립 세션(Fresh 3k)은 전체 입력 비용이 **15,000 크레딧**(Sol 캐시의 1/26)에 불과하여 다중 턴 반복 검증 시 비용 방어력이 극대화됩니다.
...
| 에이전트의 핑계 | 현실 및 불변 규칙 |
| :--- | :--- |
| *"10줄 안팎의 작업이라 캡슐 만들고 spawn하는 것보다 직접 고치는 게 빠릅니다"* | **금지.** 10줄 이상 또는 테스트 루프는 이미 다운시프트 손익분기점(BEP)을 초과했습니다. Luna Light(1.00×)로 위임해야 Sol Medium(18.04×)의 세션 누적 비용을 실질 **94.5% 절감**합니다. |
| *"지금 한창 구현 흐름 중이니 이번 한 번만 직접 코딩할게요"* | **금지.** '이번 한 번만'은 항상 세션 전체의 전면 자가 구현으로 변질됩니다. 코드 수정 도구를 호출하기 전에 즉시 Task Capsule을 작성하십시오. |
| *"Luna가 복잡한 로직을 풀 수 있게 reasoning을 High로 올릴게요"* | **비용 폭탄 & 비효율.** 실질 요율 기준 Luna High(6.00×)는 Terra Medium(5.35×)보다 11% 더 비쌉니다. 로컬 추론이 필요하면 Luna High를 쓰지 말고 더 똑똑하고 저렴한 **Terra Medium(5.35×)**으로 라우팅하십시오. |
...
- ❌ Luna에게 `high` reasoning effort를 지정함 (Luna High 6.00× > Terra Medium 5.35× 비효율. 복잡한 작업은 Terra Medium으로 스위칭)
