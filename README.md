...
일반적인 non-trivial 구현·수정·디버깅·테스트·저장소 작업에서는 Codex가 이 Skill을 자동으로 고려할 수 있습니다. 이때 Skill 활성화는 Child 위임을 뜻하지 않으며, 내부 Gate가 안전성·권한·경제성을 평가한 뒤 조건을 충족하지 않으면 Parent Direct로 계속 수행합니다.
