...
#### 3) 단가 결합 4대 운용 규칙
1. **Luna-First 원칙 (99% 절감)**: Luna `low`(지수 30)는 Sol Medium(지수 3,075) 대비 **1/102.5**로 비용이 거의 증발합니다. 10줄 이상의 기계적 구현/테스트는 무조건 Luna `low`를 디폴트로 호출합니다.
2. **Luna Medium(106) vs Terra Medium(738)**: 순수 토큰 수는 Terra가 적지만 단가가 10배 차이나므로, 실제 크레딧은 **Luna Medium이 Terra Medium보다 7배 저렴**합니다. 경량 로컬 탐색도 Luna Medium을 우선 고려합니다.
3. **Terra의 정체성 (75% 절감형 고지능 워커)**: Terra는 초저가 모델이 아니며, Sol 대비 약 76%를 아끼는 모델입니다. 복잡한 로컬 알고리즘/클래스 구조 설계에 한해 선택적으로 투입합니다.
4. **세션 누적 캐시 비용 방어**: Sol 메인 세션(40k 토큰)의 턴당 캐시 비용(400,000 크레딧) 대비, Luna 독립 세션(Fresh 3k)은 전체 입력 비용이 **15,000 크레딧**(1/26)에 불과하여 다중 턴 검증 시 비용 방어력이 극대화됩니다.
