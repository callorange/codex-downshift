...
일반적인 non-trivial 구현·수정·디버깅·테스트·저장소 작업에서는 Codex가 이 Skill을 자동으로 고려할 수 있습니다. 이때 Skill 활성화는 Child 위임을 뜻하지 않으며, 내부 Gate의 조건을 충족하지 못한 candidate만 Parent Direct로 수행합니다. 이후 별개 candidate는 다시 평가합니다.

명시 호출은 자동 선택에 의존하지 않고 Skill을 사용하려는 경우에 선택할 수 있습니다. Codex에 스킬 이름과 함께 작업 범위, 유지할 동작, 검증 기준을 전달합니다. 다음은 요청 예시이며 경로와 명령은 작업 프로젝트에 맞게 바꿉니다.
...
부모는 먼저 자신이 맡아야 할 판단을 해결하고 남은 실행 작업을 범위와 완료 기준이 정해진 후보로 식별합니다. 사용자 전체 요청이 자동으로 후보 하나가 되지는 않습니다. 위임하는 경우 Task Capsule 작성과 자식 모델 선택까지 수행하므로 사용자가 spawn 파라미터를 직접 작성할 필요는 없습니다. 준비·검증 비용에 비해 실행량이 작으면 해당 후보를 **Parent Direct**로 처리하는 것이 정상입니다.
...
라우팅을 실제 평가한 candidate마다 최종 결정을 한 번 표시합니다. 하나의 요청에서도 후보별 notice가 나올 수 있으며, 후보 내부 단계마다 반복하지 않습니다. 다음은 출력 예시입니다.
